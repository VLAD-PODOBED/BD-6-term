...
MATCH_RECOGNIZE (
    ...
    ORDER BY tstamp
    MEASURES
        FIRST(UP.tstamp) AS start_peak_stamp,
        LAST(UP.tstamp) AS end_peak_stamp,
        FIRST(UP.price) AS start_peak_price,
        LAST(UP.price) AS end_peak_price
    ONE ROW PER MATCH
...
WHERE MR.end_peak_price = (
    SELECT MAX(price)
    FROM ticker
    WHERE symbol = MR.symbol
)
AND MR.end_peak_stamp BETWEEN (
    SELECT MIN(tstamp)
    FROM ticker
    WHERE symbol = MR.symbol
) AND (
